...
class Solution:
    ...
    def ladderLength(self, start, end, dict):
        ...
        #通过pre来找出一条路径， prev={现在的node : 现在node的前一个node}
        #e.g: prev={hot:hit}, hot是来自于hit
        prev = {}
    ...
    def printPath(self, prev, end):
        
        #现在path里装最后的node,通过prev一个一个找到它的前面的node,并打印出来
        node = end
        path = []
        while node in prev:
            path.append(node)
